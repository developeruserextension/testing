...
class Process
{
            ...
            try {
                $this->shell->execute(
                    str_replace(
                        '%f',
                        $imagePath,
                        JpegOptimization::TOOLS[$queue->getTool()]['command']
                    )
                );
            } catch (\Exception $e) {
                null;
            ...
            try {
                $this->shell->execute(
                    str_replace(
                        '%f',
                        $imagePath,
                        PngOptimization::TOOLS[$queue->getTool()]['command']
                    )
                );
            } catch (\Exception $e) {
                null;
            ...
            try {
                $this->shell->execute(
                    str_replace(
                        '%f',
                        $imagePath,
                        GifOptimization::TOOLS[$queue->getTool()]['command']
                    )
                );
            } catch (\Exception $e) {
                null;
    ...
    public function getWebpFileName($imagePath, \Amasty\PageSpeedOptimizer\Api\Data\QueueInterface $queue)
    {
        ...
        return str_replace('.' . $queue->getExtension(), '.webp', $webPPath);
    }
