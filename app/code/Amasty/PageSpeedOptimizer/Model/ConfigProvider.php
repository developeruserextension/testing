...
class ConfigProvider extends \Amasty\Base\Model\ConfigProviderAbstract
{
    ...
    {
        $excludeUrls = $this->getValue(self::EXCLUDE_URLS_FROM_MERGE_BUNDLE);
        if (empty($excludeUrls)) {
            return [];
        }

        return array_map('trim', explode(PHP_EOL, $excludeUrls));
    }
    ...
    {
        $excludeUrls = $this->getValue(self::EXCLUDE_URLS_FROM_MERGE_CSS);
        if (empty($excludeUrls)) {
            return [];
        }
    ...
    {
        $ignoreList = $this->getValue(self::MOVE_JS_EXCLUDE_URL);
        if (empty($ignoreList)) {
            return [];
        }
    ...
    {
        $ignoreList = $this->getValue(self::MOVE_JS_EXCLUDE_PART);
        if (empty($ignoreList)) {
            return [];
        }
    ...
    {
        $ignoreList = $this->getValue(self::REPLACE_IMAGES_USING_USER_AGENT_IGNORE_LIST);
        if (empty($ignoreList)) {
            return [];
        }
