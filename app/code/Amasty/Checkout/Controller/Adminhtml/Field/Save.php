...
class Save extends FieldAction
{
    ...
    private function createCacheEavCustomAttributes()
    {
        ...
        for ($i = 1; $i <= $countCustomFields; $i++) {
            $constNameCustomField = '\Amasty\Checkout\Api\Data\CustomFieldsConfigInterface::CUSTOM_FIELD_' . $i . '_CODE';
    ...
    private function saveField($field, $fieldData)
    {
        ...
        if ($this->isCustomFieldAttribute($fieldData['attribute_id'])) {
            ...
            if ($attribute) {
                $this->eavSetup->updateAttribute($attribute['entity_type_id'], $attribute['attribute_id'], 'frontend_label', $fieldData['label']);
            }
        }
